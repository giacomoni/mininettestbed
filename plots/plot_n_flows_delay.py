...
for FLOWS in [3]:
   for DELAY in [10]:
      for QMULTS in [0.2]:
            ...
            for run in RUNS:
               ...
               for n in range(FLOWS):
                  ...
                  if os.path.exists(PATH + '/csvs/x%s.csv' % (n+1)):
                     ...
                     receiver_total = receiver_total[(receiver_total['time'] >= (start_time+n*100)) & (receiver_total['time'] <= (end_time+n*100))]
         ...
         LINEWIDTH = 1
         fig, axes = plt.subplots(nrows=1, ncols=3, figsize=(7, 0.5))
         ...
         plt.savefig('goodput_over_time_%smbps_%sms_%sbuf_%s_no_offload_routers.png' % (BW,DELAY,QMULTS,FLOWS), dpi=720)
