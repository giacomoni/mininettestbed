...
def run_emulation(topology, protocol, params, bw, delay, qsize_in_bytes, tcp_buffer_mult=3, run=0, aqm='fifo', loss=None, n_flows=2):
    ...
    small_bdp_in_bytes = int(bw * (2 ** 20) * 2 * 10 * (10 ** -3) / 8)
    qsize_in_bytes = max(int(qmult * small_bdp_in_bytes), 1510)

    
    net = Mininet(topo=topo)
    path = "/home/luca/mininettestbed/results_fairness_inter_rtt_async_2_correct/%s/%s_%smbit_%sms_%spkts_%sloss_%sflows_%stcpbuf_%s/run%s" % (aqm, topology, bw, delay, int(qsize_in_bytes/1500), loss, n_flows, tcp_buffer_mult, protocol, run)
    mkdirp(path)
    ...
    network_config = [NetworkConf('c2', 's1', None, 2*delay - 20, 3*bdp_in_bytes, False, 'fifo', loss),
                      NetworkConf('s1', 's2', None, 2*10, 3*bdp_in_bytes, False, 'fifo', loss),
