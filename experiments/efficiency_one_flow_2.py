...
def run_emulation(topology, protocol, params, bw, delay, qsize_in_bytes, tcp_buffer_mult=3, run=0, aqm='fifo'):
    ...
    path = "/home/luca/mininettestbed/results_one_flow_2/%s/%s_%smbit_%sms_%spkts_%stcpbuf_%s/run%s" % (aqm, topology, bw, delay, int(qsize_in_bytes/1500), tcp_buffer_mult, protocol, run)
    mkdirp(path)
