...
def run_emulation(topology, protocol, params, bw, delay, qsize_in_bytes, tcp_buffer_mult=3, run=0, aqm='fifo', loss=None, n_flows=2):
    ...
    fixed_bdp_in_bytes = int(bw * (2 ** 20) * 2 * 80 * (10 ** -3) / 8)
    qsize_in_bytes = 2*1024*1024
    
    net = Mininet(topo=topo)
    path = "/home/luca/mininettestbed/results_fairness_inter_rtt_async_2MB/%s/%s_%smbit_%sms_%spkts_%sloss_%sflows_%stcpbuf_%s/run%s" % (aqm, topology, bw, delay, int(qsize_in_bytes/1500), loss, n_flows, tcp_buffer_mult, protocol, run)
    mkdirp(path)
    ...
    network_config = [NetworkConf('c1', 's1', None, 2*delay, 3*bdp_in_bytes, False, 'fifo', loss),
                      NetworkConf('c2', 's1', None, 2*80, 3*bdp_in_bytes, False, 'fifo', loss),
    ...
    if n_flows == 1:
        ...
        traffic_config = [TrafficConf('c1', 'x1', 0, 200, protocol),
                           TrafficConf('c2', 'x2', 50, 200, protocol)]
    elif n_flows == 3:
