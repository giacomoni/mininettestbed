...
def run_emulation(topology, protocol, params, bw, delay, qsize_in_bytes, tcp_buffer_mult=3, run=0, aqm='fifo', loss=None, n_flows=2):
    ...
    path = "/home/luca/mininettestbed/results_fairness_inter_rtt_async_correct/%s/%s_%smbit_%sms_%spkts_%sloss_%sflows_%stcpbuf_%s/run%s" % (aqm, topology, bw, delay, int(qsize_in_bytes/1500), loss, n_flows, tcp_buffer_mult, protocol, run)
    mkdirp(path)
    ...
    network_config = [NetworkConf('c1', 's1', None, 2*delay - 20, 3*bdp_in_bytes, False, 'fifo', loss),
