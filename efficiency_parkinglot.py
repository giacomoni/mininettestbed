...
def run_emulation(topology, protocol, params, bw, delay, qsize_in_bytes, tcp_buffer_mult=3, run=0, aqm='fifo', loss=None, n_flows=2):
    ...
    path = "/home/luca/mininettestbed/nooffload/parkinglot_test/%s/%s_%smbit_%sms_%spkts_%sloss_%sflows_%stcpbuf_%s/run%s" % (aqm, topology, bw, delay, int(qsize_in_bytes/1500), loss, n_flows, tcp_buffer_mult, protocol, run)
    mkdirp(path)
    ...
    network_config = [NetworkConf('s1', 's2', None, 2*delay, 3*bdp_in_bytes, False, 'fifo', loss),
                      NetworkConf('s4', 's3', None, 2*delay, 3*bdp_in_bytes, False, 'fifo', loss),
                      NetworkConf('s2', 's3', bw, None, qsize_in_bytes, False, aqm, None),
                      NetworkConf('s3', 's5', bw, None, qsize_in_bytes, False, aqm, None)]
    
   # TODO: the current code only accepts configuration sorted by their starting time!!!!
    traffic_config = [TrafficConf('c1', 'x1', 0, 150, protocol)]
    previous_start = 0
    for i in range(2,6):
        wait_for = random.randint(1,20)
        traffic_config.append(TrafficConf('c%s' % i, 'x%s' % i, previous_start + wait_for, 150 - previous_start - wait_for, protocol))
        previous_start+=wait_for
    ...
    monitors = ['s2-eth2', 's3-eth2', 'sysstat']
    if protocol != 'aurora':
